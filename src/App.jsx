  ...
  const handleAddVehicle = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMenuData((prev) => ({
          ...prev,
          [selectedMenu]: [...prev[selectedMenu], data],
        }));
        setVehicleInfo({ name: "", description: "", image: "", manufacturer: "", year: "" });
      } else {
  ...
  const handleDeleteVehicle = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        setMenuData((prev) => ({
          ...prev,
          [selectedMenu]: prev[selectedMenu].filter((vehicle) => vehicle.id !== id),
        }));
      } else {
          ...
          <section className="p-6 grid gap-6 lg:grid-cols-2 md:grid-cols-1">
            {menuData[selectedMenu].map((item) => (
              <Card
                ...
                image={item.image}
                handleDelete={handleDeleteVehicle} // Pass the delete handler as a prop
              />
            ))}
          </section>
        )}
